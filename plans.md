The thickness of the ribbon needs to be parameterized via a slider in the tools area.

drawings of all kinds ( gesture, walk, text) should be automatically saved to R2. they can then be surfaced in a drawing browser allowing the user to remix old ideas. we may be able to get some implementation layout concepts from the existing texture browse library.
