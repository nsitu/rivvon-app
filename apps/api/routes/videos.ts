...
function serializeVideo(row: Record<string, any>) {
    ...
    const { export_settings_json: _ignored, ...video } = row;
    return {
        ...video,
...
videoRoutes.get('/:id', optionalSession, async (c) => {
    ...
    const video = await c.env.DB.prepare(`${VIDEO_SELECT} WHERE v.id = ? AND v.status = 'complete'`)
        .bind(videoId)
    ...
    return jsonResponse({ video: serializeVideo(video) });
});
