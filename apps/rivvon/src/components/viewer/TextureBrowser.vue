<script setup>
    ...
    const loadTextureCacheCoordinator = createLazyLoader(() => import('../../services/textureCacheCoordinator.js'));
    ...
    async function performDeriveVariant() {
        ...
        try {
            ...
            if (!isLocalTexture(textureToDerive.value) && sourceBundle.downloadedSourceBlobs) {
                const { cacheCloudTextureInBackground } = await loadTextureCacheCoordinator();
                cacheCloudTextureInBackground({
                    texture: sourceTexture,
                    textureSet: sourceBundle.sourceTextureSet,
                    cacheCloudTexture,
                    ktx2Blobs: sourceBundle.downloadedSourceBlobs,
                    onPersisted: () => {
                        cachedCloudIds.value = new Set([...cachedCloudIds.value, sourceTexture.id]);
                    },
                    logPrefix: '[TextureBrowser]',
                });
            }
    ...
    const previewIsBusy = computed(() => previewLoading.value || Boolean(previewViewerRef.value?.isBusy));
        ...
        try {
            ...
            const resolvedTexture = await resolveTextureLoadTarget({
                ...
                onInvalidCachedLocal: async ({ cloudTextureId }) => {
                    ...
                    cachedCloudIds.value = new Set([...cachedCloudIds.value].filter((id) => id !== cloudTextureId));
                },
            ...
            if (resolvedTexture.kind === 'remote' || resolvedTexture.kind === 'session') {
                ...
                previewBlobURLs.value = createObjectUrlsFromLocalTiles(resolvedTexture.localTiles || []);
            }
        } catch (err) {
...
<style scoped>
    ...
    .texture-preview-panel :deep(.tile-linear-viewer) {
        width: 100%;
        align-self: stretch;
        margin-block: 0;
    }
