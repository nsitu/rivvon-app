...
import {
  buildRibbonTileIntervals,
  summarizeRibbonTileIntervals,
} from "./ribbonTileLayout.js";
...
export function normalizeTubeRadialSegments(value) {
  ...
  const clamped = Number.isFinite(parsed)
    ? Math.max(
        MIN_TUBE_RADIAL_SEGMENTS,
        Math.min(MAX_TUBE_RADIAL_SEGMENTS, parsed),
      )
    : 8;

  return clamped % 2 === 0
    ? clamped
    : Math.min(MAX_TUBE_RADIAL_SEGMENTS, clamped + 1);
}
...
export function getMirroredTubeTextureV(radialIndex, radialSegments) {
  ...
  const circumferenceT = Math.max(
    0,
    Math.min(1, Number(radialIndex) / safeSegments),
  );
  return circumferenceT <= 0.5 ? circumferenceT * 2 : 2 - circumferenceT * 2;
}
...
export class Ribbon {
  ...
  syncWaveSpeedToLayerCycle() {
    ...
    const undulationPeriod =
      this.tileManager.getOptimalUndulationPeriod?.(3.0) || 3.0;
  ...
  setHelixOptions(options = {}) {
    ...
    if (options.helixRadius !== undefined)
      this.helixRadius = options.helixRadius;
    if (options.helixPitch !== undefined) this.helixPitch = options.helixPitch;
    if (options.helixStrandWidth !== undefined)
      this.helixStrandWidth = options.helixStrandWidth;
    if (options.cornerNarrowingEnabled !== undefined)
      this.cornerNarrowingEnabled = !!options.cornerNarrowingEnabled;
    if (options.surfaceMode !== undefined)
      this.surfaceMode = options.surfaceMode === "tube" ? "tube" : "ribbon";
    if (
      options.tubeRadiusScale !== undefined &&
      Number.isFinite(options.tubeRadiusScale)
    ) {
      this.tubeRadiusScale = Math.max(
        0.1,
        Math.min(1, options.tubeRadiusScale),
      );
    }
    if (options.tubeRadialSegments !== undefined) {
      this.tubeRadialSegments = normalizeTubeRadialSegments(
        options.tubeRadialSegments,
      );
    }
    if (
      options.tubeTextureJoinOffsetDegrees !== undefined &&
      Number.isFinite(options.tubeTextureJoinOffsetDegrees)
    ) {
      this.tubeTextureJoinOffsetDegrees = Math.max(
        0,
        Math.min(359, Math.round(options.tubeTextureJoinOffsetDegrees)),
      );
    }
    if (options.undulationEnabled !== undefined)
      this.undulationEnabled = !!options.undulationEnabled;
    if (options.ribbonPathAlignmentMode !== undefined)
      this.ribbonPathAlignmentMode =
        options.ribbonPathAlignmentMode || "center";
    if (options.sphericalProjectionEnabled !== undefined)
      this.sphericalProjectionEnabled = !!options.sphericalProjectionEnabled;
    if (
      options.sphericalProjectionRadius !== undefined &&
      Number.isFinite(options.sphericalProjectionRadius) &&
      options.sphericalProjectionRadius > 0
    ) {
  ...
  _assignCapAttributesToMesh(mesh, options = {}) {
    ...
    const startStyle =
      this.surfaceMode !== "tube" && options.start
        ? this._getCapStyleCode()
        : 0;
    const endStyle =
      this.surfaceMode !== "tube" && options.end ? this._getCapStyleCode() : 0;
    ...
    geometry.setAttribute(
      "capStartStyle",
      new THREE.BufferAttribute(startValues, 1),
    );
    geometry.setAttribute(
      "capEndStyle",
  ...
  _setLayoutMetadata({
    ...
    const capSegmentsTouched = intervals.reduce((count, interval) => {
      const capOptions = this._getIntervalCapOptions(
        interval,
        pathLength,
        capWorldLength,
      );
  ...
  _buildRibbonSegmentFromInterval({
    ...
    const startT = this._distanceToGlobalT(
      interval.visibleStartDistance,
      this.pathLength,
    );
    const endT = this._distanceToGlobalT(
      interval.visibleEndDistance,
      this.pathLength,
    );
  ...
  _updateRibbonSegmentFromInterval(
    mesh,
    {
      curve,
      interval,
  ...
  _projectDirectionOntoSurface(
    ...
    target
      .copy(direction)
      .addScaledVector(surfaceNormal, -direction.dot(surfaceNormal));
  ...
  _computeSphericalSideVector(
    surfaceNormal,
    tangent,
    fallbackSideVector = null,
    target = new THREE.Vector3(),
  ) {
    ...
    if (target.lengthSq() < 1e-10 && fallbackSideVector) {
      target
        .copy(fallbackSideVector)
        .addScaledVector(surfaceNormal, -fallbackSideVector.dot(surfaceNormal));
    }
  ...
  _sampleSphericalTangent(
    ...
    const prevPoint = this._projectCurvePointOntoSphere(
      curve.getPoint(prevT),
      new THREE.Vector3(),
    );
    const nextPoint = this._projectCurvePointOntoSphere(
      curve.getPoint(nextT),
      new THREE.Vector3(),
    );
  ...
  buildSegmentedRibbon(points, width, time) {
    ...
    const useCornerNarrowing =
      this.cornerNarrowingEnabled &&
      !this.helixMode &&
      this.surfaceMode !== "tube";
    const cornerIntervalFn = useCornerNarrowing
      ? (localT, globalT, frame) =>
          this._getCurvatureRetainedInterval(frame.curvature, 1)
      : null;
  ...
  buildPooledSegmentedRibbon(points, width, time, options = {}) {
    const maxSegmentCount = Math.max(
      1,
      Math.ceil(Number(options.maxSegmentCount) || 1),
    );
    ...
    const activeSegmentCount = Math.max(
      1,
      intervals.filter((interval) => interval.visible).length,
    );
    const frameSamples = this.createFrameSamples(
      curve,
  ...
  updatePooledSegmentedRibbon(points, width, time, options = {}) {
    const maxSegmentCount = Math.max(
      1,
      Math.ceil(
        Number(options.maxSegmentCount) || this.meshSegments.length || 1,
      ),
    );
  ...
  _getBasePointsPerSegment() {
    ...
    return this.helixMode
      ? Math.max(
          ...
          Math.ceil(basePointsPerSegment * (1 + this.helixPitch / 4)),
        )
      : basePointsPerSegment;
  }
  ...
  createFrameSamples(curve, sampleCount) {
    if (this.sphericalProjectionEnabled) {
      ...
      for (let i = 0; i < sampleCount; i++) {
        ...
        samplePoints.push(
          this._projectCurvePointOntoSphere(
            curve.getPoint(t),
            new THREE.Vector3(),
          ),
        );
      ...
      for (let i = 0; i < sampleCount; i++) {
        ...
        const surfaceNormal = point
          .clone()
          .sub(this.sphericalProjectionCenter)
          .normalize();
        ...
        tangents.push(
          this._projectDirectionOntoSurface(tangent, surfaceNormal, tangent),
        );
      }
      ...
      const rawCurvatures = tangents.map((_, index) => {
        ...
        const rightIndex = Math.min(
          sampleCount - 1,
          index + CURVATURE_LOOKAHEAD_SAMPLES,
        );
    ...
    for (let i = 0; i < sampleCount; i++) {
      ...
      if (i === 0) {
        ...
        if (c1 < 1e-10) {
          ...
          const reflectedNormal = prevNormal
            .clone()
            .addScaledVector(v1, (-2 / c1) * v1.dot(prevNormal));
          const reflectedTangent = prevTangent
            .clone()
            .addScaledVector(v1, (-2 / c1) * v1.dot(prevTangent));
  ...
  _sampleFrame(curve, frameSamples, globalT) {
    ...
    if (this.sphericalProjectionEnabled) {
      const point = this._projectCurvePointOntoSphere(
        curve.getPoint(clampedT),
        new THREE.Vector3(),
      );
      const normal = point
        .clone()
        .sub(this.sphericalProjectionCenter)
        .normalize();
      const tangent = this._sampleSphericalTangent(
        curve,
        clampedT,
        frameSamples.length,
        normal,
      ...
      const fallbackSideVector = frameSamples[lowerIndex].binormal
        .clone()
    ...
    const normal = frameSamples[lowerIndex].normal
      .clone()
  ...
  _setAnimatedVertexPosition(
    ...
    const wavePhase = this.undulationEnabled
      ? Math.sin(
          frame.arcLength * this.waveFrequency * Math.PI * 2 +
            time * this.waveSpeed,
        ) * this.waveAmplitude
      : 0;
    ...
    if (this.helixMode) {
      const helixAngle =
        frame.globalT * this.helixPitch * Math.PI * 2 + strandOffset;
      ...
      scratch.acrossDir
        .crossVectors(frame.tangent, scratch.radialDir)
        .normalize();

      target
        .copy(scratch.helixCenter)
        .addScaledVector(scratch.acrossDir, scaledAcross * strandWidth);
      return target;
    }
    ...
    target
      .copy(frame.point)
      .addScaledVector(scratch.animatedNormal, scaledAcross * width);
    return target;
  }

  _setAnimatedTubeVertexPosition(
    target,
    frame,
    radius,
    theta,
    ...
    scratch.widthDirection.copy(frame.widthDirection || frame.normal);
    scratch.animatedNormal
      .copy(scratch.widthDirection)
      .applyAxisAngle(frame.tangent, wavePhase);
    scratch.animatedBinormal
      .crossVectors(frame.tangent, scratch.animatedNormal)
      .normalize();
  ...
  _createSegmentMesh(geometry, segmentIndex, overrideTileManager = null) {
    ...
    if (
      activeTileManager &&
      typeof activeTileManager.getMaterial === "function"
    ) {
      material =
        activeTileManager.getMaterial(textureIndex, {
  ...
  _getCurvatureRetainedInterval(curvature = 0, maxWidthScale = 1) {
    ...
    const retainedWidth = clamp01(
      Math.min(clampedMaxWidthScale, curvatureWidth),
    );
  ...
  createStripRibbonSegmentWithCache(
    curve,
    startT,
    endT,
    width,
  ...
  createStripRibbonSegmentGeometryWithCache(
    curve,
    startT,
    endT,
    width,
    ...
    const capEndUs = [];
    const indices = [];
    const uStart = Number.isFinite(geometryOptions.uStart)
      ? geometryOptions.uStart
      : 0;
    const uEnd = Number.isFinite(geometryOptions.uEnd)
      ? geometryOptions.uEnd
      : 1;
    const capWorldLength = Math.max(
      0.0001,
      Number(geometryOptions.capWorldLength) || 0.0001,
    );
    const pathLength = Math.max(
      0,
      Number(geometryOptions.pathLength) || this.pathLength || 0,
    );
    const tileWorldLength = Math.max(
      0.0001,
      Number(geometryOptions.tileWorldLength) || width || 1,
    );
    ...
    geometry.setAttribute(
      "edgeNoiseU",
      new THREE.Float32BufferAttribute(edgeNoiseUs, 1),
    );
    geometry.setAttribute(
      "capStartU",
      new THREE.Float32BufferAttribute(capStartUs, 1),
    );
    geometry.setAttribute(
      "capEndU",
      new THREE.Float32BufferAttribute(capEndUs, 1),
    );
  ...
  createTubeRibbonSegmentWithCache(
    curve,
    startT,
    endT,
    width,
  ...
  createTubeRibbonSegmentGeometryWithCache(
    curve,
    startT,
    endT,
    width,
    ...
    const tubeTextureJoinOffsetRadians =
      ((this.tubeTextureJoinOffsetDegrees || 0) / 180) * Math.PI;
  ...
  createMaskedRibbonSegmentWithCache(
    curve,
    startT,
    endT,
    width,
  ...
  updateWaveAnimation(time) {
    ...
    if (
      this.helixMode &&
      this.helixMeshSegmentsB.length > 0 &&
      this._segmentCacheB.length > 0
    ) {
      this._updateStrandAnimation(
        this.helixMeshSegmentsB,
        this._segmentCacheB,
        time,
      );
  ...
  createRibbonFromDrawing(drawPoints) {
    ...
    const points3D = normalizedPoints.map(
