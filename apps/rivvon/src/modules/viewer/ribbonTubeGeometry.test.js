...
describe('tube ribbon geometry', () => {
    ...
    it('builds closed tube side walls while keeping planar masks away from the seams', () => {
        ...
        expect(Array.from(geometry.attributes.maskV.array).every((value) => value === 0.5)).toBe(true);
    ...
    it('selects tube geometry through the normal build path and disables planar cap masks', () => {
        ...
        expect(Array.from(geometry.attributes.capStartStyle.array).every((value) => value === 0)).toBe(true);
        expect(Array.from(geometry.attributes.capEndStyle.array).every((value) => value === 0)).toBe(true);
