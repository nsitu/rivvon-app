/**
 * Linear Tile Renderer (WebGPU) - Displays KTX2 tiles in a linear, document-like layout
 * Uses Three.js WebGPU backend with TSL node materials for better ASTC array texture support,
...
export class TileLinearRendererWebGPU {
    ...
    async init(container, options = {}) {
        ...
        // Create WebGPU renderer with alpha for transparency
        ...
        console.log('[TileLinearRenderer WebGPU] Initialized with options:', {
            tileSize,
            maxViewportWidth,
            maxViewportHeight,
            flowDirection
    ...
    updateLayout() {
        ...
        console.log('[TileLinearRenderer WebGPU] Layout updated:', {
    ...
    preallocatePlaceholders(count) {
        for (let i = 0; i < count; i++) {
            ...
            this.scene.add(mesh);
            this.tiles.set(tileId, { mesh, texture: null, material, geometry, layerUniform: null, isPlaceholder: true });
        }
    ...
    /**
     * Create TSL node material for texture array
     * Uses Three.js Shading Language for WebGPU-native material
     * @param {THREE.Texture} arrayTexture - KTX2 array texture
     * @returns {{ material: THREE.NodeMaterial, layerUniform }}
     */
    createArrayMaterial(arrayTexture) {
        // Create a uniform node for the current layer
    ...
    async upsertTile(tileId, blobURL) {
        return new Promise((resolve, reject) => {
            ...
            this.ktx2Loader.load(
                ...
                undefined,
                (error) => {
                    console.error(`[TileLinearRenderer WebGPU] Failed to load tile ${tileId}:`, error);
                    reject(error);
                }
            );
        });
    ...
    /**
     * Start animation loop (uses WebGPU setAnimationLoop)
     */
    startAnimation() {
        ...
        console.log('[TileLinearRenderer WebGPU] Animation loop started');
    }
    ...
    dispose() {
        ...
        console.log('[TileLinearRenderer WebGPU] Disposed');
    }
}
